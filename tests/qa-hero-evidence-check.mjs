...
try {
  ...
  await check(await evidence.count() === 1, "히어로에 최근 답변 기록이 표시된다");
  const title = (await evidence.locator("strong").innerText()).trim();
  await evidence.click();
  await check(await page.locator(".detail-modal").isVisible(), "히어로 기록에서 해당 질문 상세를 연다");
  ...
  await check((await page.locator(".hero-evidence button").filter({ hasText: title }).count()) === 0, "차단한 작성자의 답변 기록을 즉시 숨긴다");
} finally {
  await browser.close();
}
