...
try {
  ...
  await page.getByPlaceholder("태그, 기술, 막힌 장면 검색").fill("rag");
  await check(await page.locator(".question-row").count() === 1, "태그·기술 검색");
  await page.getByPlaceholder("태그, 기술, 막힌 장면 검색").fill("");
  ...
  await check(await page.locator(".question-row").count() === 1, "내 막힘 필터 자동 전환");
