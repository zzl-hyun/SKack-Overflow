...
try {
  ...
  await check((await phrase.innerText()).includes("혼자 끙끙 말고") && (await phrase.innerText()).includes("SKACK"), "두 번째 SKACK 캐치프레이즈로 순환한다");
  await standardPage.waitForFunction(() => document.querySelector(".hero-phrase")?.textContent?.includes("아는건 나누고") === true, undefined, { timeout: 5000 });
  await check((await phrase.innerText()).includes("아는건 나누고") && (await phrase.innerText()).includes("모르는 건") && (await phrase.innerText()).includes("SKACK"), "세 번째 SKACK 캐치프레이즈로 순환한다");
  ...
  await check((await reducedPhrase.innerText()) === firstPhrase && firstPhrase.includes("막히면") && firstPhrase.includes("SKACK"), "모션 감소 환경에서는 첫 캐치프레이즈를 고정한다");
} finally {
  await browser.close();
}
